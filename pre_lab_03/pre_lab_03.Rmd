...
**Answer** There is a warning aboutretriving full specification for the data
...
**Answer** its a chr and it follow with mdy in order by month and day with a time stamp of 12:00
...
**Task** Run the following codeblock to group by the vendor's zip code and show any that are less than five characters in length. How many zip codes do not have five characters? What do you think explains those records? **Answer** 320 zipcodes have less then 5, smaller neighborhoods or communities perhaps
...
**Task** Run the following codeblock and describe below what each line is doing. Looking at the results, what jumps out at you as potentially problematic? **Answer** I noticed that in 14 years that the grant doubled in from the time in had started
...
**Task** Run the following code to arrange the grants & loan data so that the smallest amount is first. Describe what you see. '**Answer** I see that the amount is the highest in the middle of the data period when i was expecting it to be in the end which is a little surprising
...
**Task** Run the following codeblock to use the `clean_names()` function from janitor to standardize column names. How does it change the headers? Answer below. **Answer** There's underscores instead of spaces in the columns
...
its different for each one but it seems like it could be a lot but 58 rows in total
...
**Task** Run the following codeblock to use distinct() to get rid of duplicate rows. How many rows does the new dataframe have? 
Answer below. **Answer** 1000
...
**Task** Run the following codeblock to use str_sub() to convert the ZIP codes that have nine digits to five digits, creating a new column. Look at the difference in the result - what changed? **Answer** i'm seeing that depending on the year that the nine ditgits have a lot more amount then the 5 digit
