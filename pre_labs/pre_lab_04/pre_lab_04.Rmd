...
There's a lot of repeat county's so maybe grouping by the county would be better,
```{r}
...
Some of the county's are now all capitalized instead of just the first letter.
Punctuation  are also different.
...
Standardizing text case, removing punctuation or special characters, Clustering similar names
```{r}
cleaned_md_grant_loans |>
...
n_gram_merge() is more aggressive, grouping names that look or sound similar, while key_collision_merge() takes a more cautious approach, matching names based on simpler patterns like spelling or structure.
