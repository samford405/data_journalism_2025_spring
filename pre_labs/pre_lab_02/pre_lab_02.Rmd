...
**Answer** So the results saves and doesn't disappear
...
It seems OR operator gives a more wider data
...
**Answer:** The date stretches going from spring of 2021 to spring of 2022 with lots of various dates between
...
**Answer** I think the month column is just following the flow of the total amount column with the most to the least
...
**Answer** I think it sorted the county's in ABC order then putting the amount of votes each candidate got from each county then using that for its total votes along with percent changes
...
**Answer** the answer changed because now the table is sorted by the new percentage column, instead of keeping the original order.
...
useful cause it shows the big numbers Moore got in th election
**Answer**
...
**Answer str_to_title(), Converts the first letter of each word in a string to uppercase and the rest to lowercase.

str_to_lower(), makes all things lowercase
...
**Answer** yes it descending with the total lined up with the state.
...
1.  How has the number of Journalism courses offered changed over time, and can we use mutate() to calculate the percent increase or decrease each semester?

2.  How has the average course enrollment size changed from fall to spring semesters over the years, and can we use mutate() to calculate the percent change semester-to-semester?

3.  Which instructors taught more than one course in a given semester, and can we use mutate() to calculate their share of total courses that term?
