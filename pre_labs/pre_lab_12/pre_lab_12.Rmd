...
**Task** Run the following code in the gray-colored codeblock below to extract all tables in the HTML and save it as a dataframe. What do you notice about the columns? **The columns didn’t load cleanly. Some column names are blank or labeled as X1, X2, etc., because R couldn’t read proper headers from the HTML table. The columns look messy and need to be cleaned before using the data.**
...
tags there are in the HTML source? **The output shows many list items, which means there must be multiple <ul> tags in the HTML, not just one.**
...
tags. Describe the output: **The output shows the text from every <li> on the page. It includes a long list of items, many of which are not the ones we actually need. This tells us the page contains lots of different lists, and we’re pulling all their text together, not just the specific list we’re looking for.**
...
**Task** Run the following code in the gray-colored codeblock below to make a dataframe. Describe the output: **2 columns of 94 rows of disorganized value**
