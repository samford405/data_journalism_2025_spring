...
When you have found as many as possible, save the result to a new dataframe called `pg_schools`. How many records did you find? I found 68.
...
pg_schools <- payments |>
  filter(
    str_detect(str_to_lower(`Vendor Name`), "george") &
    str_detect(str_to_lower( `Vendor Name`), "school")
  )
...
A2: 2021 was the biggest fiscal year. 2020 was the second highest which is a bit surprising because that was peak of Covid so you would think it would be lowered kinda most things were then.
...
A3. University System of Maryland Foundation Inc and UNIVERSITY OF MD COLLEGE PARK FOUNDATION INC should be made into one. UNIV OF MD FOUNDATION O and UNIVERSITY OF MD COLLEGE PARK FOUND. UNIVERSITY OF MD COLLEGE PARK FOUNDATION INC and UNIVERSITY OF MD COLLEGE PARK FOUND. I would just combine some of groups like the ones I listed because its kind of giving redundancy and its not necessary. 


```{r} 

umd_zip_grantees <- payments |>
  filter(`Vendor Zip` == "20742") |>
...
A4: It started with a huge peak in 2009 and stayed stable for years till follow then 2023 had anither big peak

```{r}
stem_cell_totals <- md_grants_loans |>
...
A5: There are 17 grantees in the College Park zip code, and many of them are connected to the University of Maryland, which makes sense since it is the largest institution in the area. To make the data clearer, I would group all University of Maryland entries and related corporations into a single category, and then create separate columns for each of the other companies or organizations. This way, the results would be easier to compare and interpret.

```{r}
umd_grants <- md_grants_loans |>
  filter(`Zip Code` == "20742") |>
  group_by(Grantee) |>
  summarise(total_amount = sum(Amount)) |>
  arrange(desc(total_amount))
