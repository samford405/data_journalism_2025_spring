...
maryland_expenses <- read_csv("GitHub/data_journalism_2025_spring/labs/lab_04/data/maryland_expenses.csv")
```
...
media_expenses_clean <- read_csv("media_expenses_clean.csv")
...
**A2.** The top spender was Peter Franchot, who began running ads in 2021.
The second biggest spenders were another governor–lieutenant governor team. Next came a PAC backing progressive candidates focused on immigrant rights and working families, followed by another gubernatorial candidate.
Fifth was a PAC supporting candidates who champion working people and “the many.” The rest of the top ten included a Harford County Executive candidate, two more gubernatorial campaigns, a county council candidate in Anne Arundel, and a Republican representing Maryland’s 6th District.
...
**A3.**
Those missing were Wes Moore and Aruna Miller, Dan Cox and his running partner, and Tom Perez and his running mate.


```{r}
  moore_ads <- media_expenses_clean |>
  filter(str_detect(`Committee Name`, "Moore")) |>
...
**A4.**
The Ball Calvin Team seems to have spent the most in total spending. Jain  Ashwani Friends of spent the most on facebook.
```{r}
media_purpose <- media_expenses_clean |>
...
**A5**I think a really interesting story would be looking at how some candidates might be getting around fully disclosing what they’re actually spending, by funneling money through third-party groups. Or even taking a closer look at who’s really doing the work behind the scenes
